...
int affiche_ecran_debut(SDL_Surface *screen, int w, int h) {
  ...
  texte = SDL_LoadBMP("./img/taquin_start.bmp");
  if (texte == NULL) {
    fprintf(stderr, "%s\n", SDL_GetError());
    exit(0);
  }
  text_pos.y = (h - texte->h)/2;
  text_pos.x = (w - texte->w)/2;
...
void affichage(SDL_Surface * screen, SDL_Surface * image, taquin *map) {
  int i, j;
